...
async function post(path, body, host) {
  ...
  if (!r.ok) {
    throw new Error(`${path}: HTTP ${r.status} ${typeof data === "string" ? data : JSON.stringify(data)}`);
  }
  return data;
}
...
function parseLessonText(texts) {
  const values = texts.sort((a, b) => a.y - b.y || a.x - b.x).map(t => clean(t.text)).filter(Boolean);
  ...
  if (values.length === 1) {
    ...
    if (compact) { subject = clean(compact[1]); className = clean(compact[2]); }
  }
...
function parseLessons(renderR) {
  ...
  for (const [parentId, texts] of textByParent) {
    ...
    diagnosticBoxes.push({ parentId, x: Math.round(x * 100) / 100, y: Math.round(y * 100) / 100, height: Math.round(height * 100) / 100, start, end, values: parsed.values });
  ...
  walk(data, o => {
    ...
    if (!a || !b) return;
    out.push({ day: Number(o.day ?? o.dayIndex ?? 0), start: `${a[1].padStart(2, "0")}:${a[2]}`, end: `${b[1].padStart(2, "0")}`, subject: clean(subject), className: clean(o.className ?? o.group ?? o.groupName ?? ""), x: 0, y: 0 });
  });
...
async function main(env) {
  const HOST = getEnv(env, "SKOLA24_HOST", HOST_DEFAULT);
  const TEACHER = String(getEnv(env, "SKOLA24_TEACHER", TEACHER_DEFAULT)).trim().toLowerCase();
  const SCHOOL_YEAR = getEnv(env, "SKOLA24_SCHOOL_YEAR", SCHOOL_YEAR_DEFAULT);
  const SCHEMA_ID = getEnv(env, "SKOLA24_SCHEMA_ID", SCHEMA_ID_DEFAULT);
  const UNIT_GUID = getEnv(env, "SKOLA24_UNIT_GUID", UNIT_GUID_DEFAULT);
  const keyR = await post("/get/timetable/render/key", {}, HOST);
  const key = unwrap(keyR)?.key;
  if (!key) throw new Error("Skola24 gav inget renderKey.");

  const selection = getEnv(env, "SKOLA24_SELECTION", encodeGuid(SCHEMA_ID));
  const unitGuid = getEnv(env, "SKOLA24_UNIT_GUID_ENCODED", encodeGuid(UNIT_GUID));
  const now = localNow();
  const today = now.getDay() === 0 ? 6 : now.getDay() - 1;
  const renderBody = {
    blackAndWhite: false, customerKey: "", endDate: null, height: 550, host: HOST, periodText: "", personalTimetable: false,
    privateFreeTextMode: null, privateSelectionMode: false, renderKey: String(key), scheduleDay: 0, schoolYear: SCHOOL_YEAR,
    selection, selectionType: 7, showHeader: false, startDate: null, unitGuid, week: week(now), width: 813, year: now.getFullYear()
  };
  const renderR = await post("/render/timetable", renderBody, HOST);
  const parsed = parseLessons(renderR);
  const todayLessons = parsed.lessons.filter(l => l.day === today).sort((a, b) => a.start.localeCompare(b.start));
  const nowMinutes = now.getHours() * 60 + now.getMinutes();
  const current = todayLessons.find(l => { const s = minutes(l.start), e = minutes(l.end); return s != null && e != null && nowMinutes >= s && nowMinutes < e; });
  let next = null;
...
export async function onRequest(context) {
  try {
    const result = await main(context.env);
    return new Response(JSON.stringify(result), { status: 200, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "Access-Control-Allow-Origin": "*" } });
  } catch (e) {
    return new Response(JSON.stringify({ error: e?.message || String(e), stack: e?.stack || null }), { status: 500, headers: { "Content-Type": "application/json; charset=utf-8", "Access-Control-Allow-Origin": "*" } });
  }
}
